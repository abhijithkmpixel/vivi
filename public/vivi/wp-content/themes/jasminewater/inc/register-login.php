...
function woocommerce_separate_login_form() {
  ...
  if ( is_user_logged_in() ) return;
  ob_start();
  $site_url = (ICL_LANGUAGE_CODE == 'ar') ? site_url()  .'\\/ar' : site_url();
  ...
  <div class="form-row form-row-wide">
    <div class="select-wrap">
      ...
      <select name="salutation" class="regular-text" id="salutation">
        ...
        <option value="business" <?php if (get_the_author_meta( 'salutation', $user->ID ) == "business") echo 'selected="selected" '; ?>>Business</option>
        <option value="individual" <?php if (get_the_author_meta( 'salutation', $user->ID ) == "individual") echo 'selected="selected" '; ?>>Individual</option>
      </select>
    </div>
  </div>
