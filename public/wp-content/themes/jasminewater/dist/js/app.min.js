...
// qty + button diable when quantity reach 9999
jQuery('.qty-selector .plus').click(function(){
    var qty_val = jQuery('.qty').val();
    if(qty_val >= 9998) {
        jQuery('.qty-selector .plus').addClass('max_qty_reach');
    } else {
        jQuery('.qty-selector .plus').removeClass('max_qty_reach');
    }
});
jQuery('.qty-selector .minus').click(function(){
    jQuery('.qty-selector .plus').removeClass('max_qty_reach');
});
// qty + button diable when quantity reach 9999 end

// delivery type validation on cart page
  jQuery(function(){
    jQuery(document).on("click",".wc-proceed-to-checkout .btn",function(){
      if( jQuery('#delivery_type').val() == 0 ) {
        jQuery('#delivery_type')[0].setCustomValidity(invalid_message);
      } else {
        jQuery('#delivery_type')[0].setCustomValidity('');
      }       
    });
    jQuery(document).on("click",".cart-action-row .btn",function(){
      jQuery('#delivery_type')[0].setCustomValidity('');     
    });
  });
// delivery type validation on cart page end