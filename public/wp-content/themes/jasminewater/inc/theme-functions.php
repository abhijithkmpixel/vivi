...
if( function_exists('acf_add_options_page') ) {
    ...
    acf_add_options_sub_page(array(
        'page_title'    => 'Mobile app Settings',
...
function sendMail() {
    global $post_type;
    if( 'postcodes' == $post_type ) {
        $page_id = get_the_ID();
        $page_title = get_the_title();
        $postcode_user_name = get_field('postcode_user_name', $page_id);
        $postcode_user_email = get_field('postcode_user_email', $page_id);
        $sendto = $postcode_user_email;
        $sendsub = 'Notify me';
        $headersfrom='';
        ...
        $sendmess = ' 
            <html> 
            <body> 
                Hi <b>'.$postcode_user_name.'</b>,
                <br><br>
                Delivery is available for your zip code '.$page_title.'.
                <br><br>
                Regards,
                Vivi
            </body> 
            </html>'; 
        wp_mail($sendto, $sendsub, $sendmess, $headersfrom);

        ?>
      <style type="text/css">
       .post-php.post-type-postcodes #publishing-action {display:none;}
      </style>
      <?php

    }
}
add_action('save_post_postcodes', 'sendMail');
...
// notify mail - zipcode end

//add body class
add_filter( 'body_class', 'custom_class' );
function custom_class( $classes ) {
    if ( is_page_template( 'template-terms-app.php' ) ) {
        $classes[] = 'mobile-app-page';
    }
    return $classes;
}
//add body class end