    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if(data.cart_total_count < 1){
          ...
          if(lang == 'rtl'){
            cart_msg += 'لا توجد أي منتجات في السلة.';
          } else {
          ...
          if(lang == 'rtl'){
            cart_msg += '<br/> <a href="'+site_url+'/cart" class="btn btn-simple btn-white">عرض السلة</a>';
          } else {
        ...
        var updatedMsg = 'Cart Updated!';
        if(lang == 'rtl'){
          updatedMsg = 'تم تحديث السلة!';
        }

        $.toast({ 
          heading: (data.cart_total_count < 1) ? updatedMsg : data.cart_total_count + '&nbsp;' + productTitle,
          text : cart_msg, 
...
jQuery(document).ajaxComplete(function(event, xhr, settings) {
  ...
  if (
    ...
    settings.url === 'en/?wc-ajax=apply_coupon' ||
    settings.url === '/en/?wc-ajax=apply_coupon' ||
    settings.url === 'en/?wc-ajax=remove_coupon' ||
    settings.url === '/en/?wc-ajax=remove_coupon' ||
    settings.url === '/en/vivi/?wc-ajax=apply_coupon' ||
    settings.url === '/en/vivi/?wc-ajax=remove_coupon'
    ) {
      location.reload();
  }
});