...
<div class="flex col2-set">
  <div class="col">
    <form class="woocommerce-form woocommerce-form-login login" name="login" method="post">
      ...
      <?php
      $redirect_url = (ICL_LANGUAGE_CODE == 'ar') ? site_url()  .'\\/ar' . '/account' : site_url() .'\\/en' . '/account';
      if(isset($_GET['callback']) == 'cart'){
        $redirect_url = (ICL_LANGUAGE_CODE == 'ar') ? site_url()  .'\\/ar' . '/account' : site_url() .'\\/en' . '/account?callback=cart';
      }
      ?>
